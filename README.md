...
* UDN Name: Deferred Merge Append Stream
* Package Name: Deferred Merge
...
When deployed for the first time into an environment ,the below stages will be executed.

* **Create Stream**
  This stage will execute a CREATE OR REPLACE statement and create a Stream in the target environment.

* **Create Target Table**
  This table is the destination where the processed data will be stored.

* **Target Table Initial Load**
   This involves populating the target table with the existing data from the source object. This step ensures that the target table starts with a complete set of data before any further changes are applied.

* **Create Hybrid View**
   This view provides access to the most up-to-date data by combining the initial data load with any subsequent changes captured by the stream. The hybrid view ensures that users  always have access to the
   latest version of the data without waiting for batch updates.

* **Create Task**
   This task wraps a merge operation that incorporates the changes captured by the stream into the target table.

* **Resume Task**
    After the task has been created it needs to be resume so that the task runs on the schedule

* **Apply Table Clustering**
   This Step will alter a cluster Key to the table if the cluster toggle is enabled.

* **Resume Recluster Clustering**
   As DML operations  are performed on a clustered table, the data in the table might become less clustered. Periodic/regular reclustering of the table is required to maintain optimal clustering.
